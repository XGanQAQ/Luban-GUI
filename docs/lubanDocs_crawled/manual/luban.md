> Source: https://www.datable.cn/docs/manual/luban.conf

Title: luban.conf | Luban

URL Source: https://www.datable.cn/docs/manual/luban.conf

Markdown Content:
定义了luban所需要的全局配置。

## 格式[​](https://www.datable.cn/docs/manual/luban.conf#%E6%A0%BC%E5%BC%8F "格式的直接链接")
...
## dataDir[​](https://www.datable.cn/docs/manual/luban.conf#datadir "dataDir的直接链接")

指定数据根目录，此项目配置不可为空。

## schemaFiles[​](https://www.datable.cn/docs/manual/luban.conf#schemafiles "schemaFiles的直接链接")
...
## groups[​](https://www.datable.cn/docs/manual/luban.conf#groups "groups的直接链接")
...
Luban的生成代码时，会从导出table出发，递归收集所有相关的enum及bean定义。 当enum和bean的group为空时，只有被导出表直接或者间接引用，才会标记它们为导出类型，为他们生成代码。如果手动设置了它们的group，不属于当前 导出目标，但它们被导出表间接引用时，会忽略group而强行导出。

如果field的group为空，则默认属于所有分组，不可配置。这是因为大多数field会属于所有分组，我们希望让field的 group都为空就能表示导出给所有目标，而不是啰嗦地设置它的分组为所有分组。

group有多个，一般项目至少包含c（即client）和s（即server）分组。如果项目有自定义配置编辑器，一般还会有一个额外的目标e（即editor）。 group的个数和命名没有限制，只不过约定俗成会这么设计。

## targets[​](https://www.datable.cn/docs/manual/luban.conf#targets "targets的直接链接")
